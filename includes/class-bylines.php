...
class Bylines {
	 ...
	 * @return array The node objects.
	 */
	private function get_node_objects( $bylines = array() ) {
		$to_return = array();
		foreach ( $bylines as $byline ) {
			...
			$node = new Node( false, $byline['termId'] );
			if ( ! is_wp_error( $node ) ) {
				$to_return[ $byline['termId'] ] = get_object_vars( $node );
			}
		}
		return $to_return;
	}
	...
	public function get() {
		...
		return $this->get_node_objects( $bylines );
	}
		...
		foreach ( $this->bylines as $term_id => $d ) {
			if ( 1 < $total && $i === $total ) {
				if ( false === $return_html ) {
					...
					$output .= ' <span class="prc-platform-node-bylines__and-separator">' . $and . '</span> ';
				}
			} elseif ( 1 < $total && 1 !== $i ) {
				if ( false === $return_html ) {
					$output .= ', ';
				} else {
					$output .= '<span class="prc-platform-node-bylines__separator">, </span>';
				}
			}
			if ( false === $return_html ) {
				$output .= $d['name'];
